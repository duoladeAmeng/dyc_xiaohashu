...
public class SegmentChainIdGenerator implements IdGenerator {
    ...
    public SegmentChainIdGenerator(SegmentAllocator segmentAllocator) {
        this(IdSegment.TIME_TO_LIVE_FOREVER, DEFAULT_SAFE_DISTANCE, segmentAllocator,
                new PrefetchWorkerExecutorService(PrefetchWorkerExecutorService.DEFAULT_PREFETCH_PERIOD, Runtime.getRuntime().availableProcessors()));
    }
    ...
    public SegmentChainIdGenerator(long idSegmentTtl,
        ...
        if (idSegmentTtl <= 0) {
            throw new IllegalArgumentException(String.format("Illegal idSegmentTtl parameter:[%s].", idSegmentTtl));
        }
        if (safeDistance <= 0) {
            throw new IllegalArgumentException("The safety distance must be greater than 0.");
        }
        ...
        while (true) {
            ...
            try {
                ...
                if (preHead.trySetNext(preChain -> generateNext(preChain, safeDistance))) {
                    ...
                    forward(nextChain);
                    log.debug("Generate [{}] - headChain.version:[{}->{}].", segmentAllocator.getNamespacedName(), preHead.getVersion(), nextChain.getVersion());
                }
            } catch (NextIdSegmentExpiredException exception) {
                log.warn("Generate [{}] - gave up this next IdSegmentChain.", segmentAllocator.getNamespacedName(), exception);
            }
    ...
    private void forward(IdSegmentChain forwardChain) {
        ...
        log.debug("Forward [{}] - [{}] -> [{}].", segmentAllocator.getNamespacedName(), headChain, forwardChain);
    ...
    private IdSegmentChain generateNext(IdSegmentChain previousChain, int segments) {
        return segmentAllocator.nextIdSegmentChain(previousChain, segments, idSegmentTtl);
    }
    ...
    public class PrefetchJob implements AffinityJob {
        ...
        public void prefetch() {
            ...
            if (hunger) {
                ...
                log.info("Prefetch [{}] - Hunger, Safety distance expansion.[{}->{}]", segmentAllocator.getNamespacedName(), prePrefetchDistance, this.prefetchDistance);
            } else {
                ...
                if (prePrefetchDistance > this.prefetchDistance) {
                    log.info("Prefetch [{}] - Full, Safety distance shrinks.[{}->{}]", segmentAllocator.getNamespacedName(), prePrefetchDistance, this.prefetchDistance);
                }
            }
            ...
            if (safeGap <= 0 && !hunger) {
                log.trace("Prefetch [{}] - safeGap is less than or equal to 0, and is not hungry - headChain.version:[{}] - tailChain.version:[{}].",
                        segmentAllocator.getNamespacedName(), availableHeadChain.getVersion(), tailChain.getVersion());
                return;
            }
        ...
        private void appendChain(IdSegmentChain availableHeadChain, int prefetchSegments) {
            log.debug("AppendChain [{}] - headChain.version:[{}] - tailChain.version:[{}] - prefetchSegments:[{}].",
                    segmentAllocator.getNamespacedName(), availableHeadChain.getVersion(), tailChain.getVersion(), prefetchSegments);
            try {
                IdSegmentChain preTail = tailChain;
                ...
                log.debug("AppendChain [{}] - restTail - tailChain.version:[{}:{}->{}] .", segmentAllocator.getNamespacedName(),
                        preTail.gap(tailChain, segmentAllocator.getStep()), preTail.getVersion(), tailChain.getVersion());
            } catch (NextIdSegmentExpiredException exception) {
                log.warn("AppendChain [{}] - gave up this next IdSegmentChain.", segmentAllocator.getNamespacedName(), exception);
            }
        }
    }
}
